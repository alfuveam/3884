	...
	if(gameState != newState)
	{
		...
		switch(newState)
		{
			...
			case GAMESTATE_SHUTDOWN:
			{
				...
				saveGameState(false);
				Dispatcher::getInstance().addTask(createTask(std::bind(&Game::shutdown, this)));
	...
	const Position& movingCreaturePos, const Position& toPos, bool delay)
{
	...
	if(!Position::areInRange<1,1,0>(movingCreaturePos, player->getPosition()) && !player->hasCustomFlag(PlayerCustomFlag_CanMoveFromFar))
	{
		...
		{
			Dispatcher::getInstance().addTask(createTask(std::bind(&Game::playerAutoWalk,
				this, player->getID(), listDir)));
			SchedulerTask* task = createSchedulerTask(std::max((int32_t)SCHEDULER_MINTICKS, player->getStepDuration()),
				std::bind(&Game::playerMoveCreature, this, playerId, movingCreatureId, movingCreaturePos, toPos, true));
	...
	else if(delay)
	{
		...
		if(delayTime > 0)
		{
			SchedulerTask* task = createSchedulerTask(delayTime,
				std::bind(&Game::playerMoveCreature, this, playerId, movingCreatureId, movingCreaturePos, toPos, false));
			player->setNextActionTask(task);
			return true;
		}
	}
...
bool Game::playerSay(uint32_t playerId, uint16_t channelId, SpeakClasses type, const std::string& receiver, const std::string& text, ProtocolGame* pg) //CAST
{
    ...
    bool logsPlayer = g_config.getBool(ConfigManager::LOG_PLAYER);
    if (logsPlayer) {	
...
bool Game::steerCreature(Creature* creature, const Position& position)
{
	...
	if(!Position::areInRange<1,1,0>(creature->getPosition(), position))
	{
		SchedulerTask* task = createSchedulerTask(std::max((int32_t)SCHEDULER_MINTICKS,
			creature->getStepDuration()), std::bind(&Game::steerCreature, this, creature, position));
...
bool Game::playerInviteToParty(uint32_t playerId, uint32_t invitedId)
{
    ...
    if(invitedPlayer == player) { //Anti-Crash Party
        return false;
	} 
	...
	for(auto stageNode : doc.child("stages").children())
	{
		if(!(stageNode.attribute("world")))
		{
			if((attr = stageNode.attribute("id")))
			{
				...
				if(!parseIntegerVec(attr.as_string(), intVector) || std::find(intVector.begin(),
			...
			if((attr = stageNode.attribute("multiplier")))
				defStageMultiplier = attr.as_float();
...
void Game::checkHighscores()
{
	...
	Scheduler::getInstance().addEvent(createSchedulerTask(tmp, std::bind(&Game::checkHighscores, this)));
}
...
void Game::prepareGlobalSave()
{
	if(!globalSaveMessage[0])
	{
		...
		Scheduler::getInstance().addEvent(createSchedulerTask(120000, std::bind(&Game::prepareGlobalSave, this)));
	}
	...
	else if(!globalSaveMessage[2])
	{
		...
		Scheduler::getInstance().addEvent(createSchedulerTask(60000, std::bind(&Game::prepareGlobalSave, this)));
	}
	else
		globalSave();
}
...
void Game::globalSave()
{
	...
	{
		//shutdown server
		Dispatcher::getInstance().addTask(createTask(std::bind(&Game::setGameState, this, GAMESTATE_SHUTDOWN)));
		return;
	}

	//close server
	Dispatcher::getInstance().addTask(createTask(std::bind(&Game::setGameState, this, GAMESTATE_CLOSED)));
	...
	Scheduler::getInstance().addEvent(createSchedulerTask(86100000, std::bind(&Game::prepareGlobalSave, this)));
	//open server
	Dispatcher::getInstance().addTask(createTask(std::bind(&Game::setGameState, this, GAMESTATE_NORMAL)));
}
